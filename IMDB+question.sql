...
+---------------+-------------------+ */
-- Type your code below:
select year, count(year) as number_of_movies from movie group by year ;
select month(date_published) as month_num, count(year) as number_of_movies from movie group by month_num order by month_num;
...
-- Type your code below:
select count(id) as number_of_movies_in_2019_USA_INDIA from movie where year=2019 and (country like'%USA%' or  country like '%INDIA%') ;
...
select distinct(genre) as list_of_Genres from genre ;
...
select genre, count(genre) as number_of_movies from genre  
...
select genre, round(avg(duration)) as avg_duration from genre g 
...
SELECT
MIN(round(avg_rating)) AS min_avg_rating,
MAX(round(avg_rating) )AS max_avg_rating,
MIN(round(total_votes)) AS min_total_votes,
MAX(round(total_votes)) AS max_total_votes,
MIN(round(median_rating)) AS min_median_rating,
MAX(round(median_rating)) AS max_median_rating
FROM ratings;
...
select title, avg_rating, dense_rank() over(order by avg_rating desc) as movie_rank from movie m
inner join ratings r on m.id = r.movie_id limit 10 ;
...
inner join genre g on m.id = g.movie_id inner join ratings r on m.id = r.movie_id 
...
inner join ratings r on m.id = r.movie_id inner join
genre g on m.id = g.movie_id where title like 'The%' and avg_rating>8 
...
SELECT COUNT(id) AS no_of_movie_released, median_rating FROM movie m 
INNER JOIN ratings r ON m.id = r.movie_id WHERE date_published BETWEEN '2018-04-01'AND '2019-04-01'
AND median_rating = 8;
...
SELECT languages, Sum(total_votes) AS total_no_of_votes FROM movie m 
INNER JOIN ratings r ON r.movie_id = m.id
WHERE languages LIKE '%Italian%' UNION SELECT languages, Sum(total_votes) AS total_No_of_votes FROM movie m 
